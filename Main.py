...
class Instance:
    ...
    def initialize_tracking(self, tracked_object_locs, tracked_axis):
        ...
        self.mag_tracker = np.zeros([int(self.extent / self.t), len(tracked_object_locs)])
        ...
        for i, axis in enumerate(self.tracked_axis):
            ...
            self.mag_tracker[0, i] = np.linalg.norm(self.tracked_objects[i].pos - self.tracked_objects[i].initial_pos)
    ...
    def simulate(self):
        ...
        for i, tick in enumerate(np.arange(self.t, self.extent, self.t)):
            ...
            for j, (rigid, axis) in enumerate(zip(self.tracked_objects, self.tracked_axis)):
                ...
                self.mag_tracker[i+1, j] = np.linalg.norm(self.tracked_objects[j].pos - self.tracked_objects[j].initial_pos)
    ...
    def graph_motion(self):
        ...
        for j, rigid in enumerate(self.tracked_objects):
            ...
            ax.plot(self.time_axis, self.mag_tracker[:, j], label=f"Mag ofRigid at {rigid.initial_pos}")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Position (m)")
        ax.set_title("Displacements in a Hookean System")
        ax.legend()
        motion_plot.show()
        m1 = self.motion_tracker[0::40, 0]
        m2 = self.motion_tracker[0::40, 1]
        m3 = self.mag_tracker[0::40, 0]
        np.savetxt('3DOFMotion.csv', np.asarray(list(zip(self.time_axis[0::30], m1, m2, m3))), delimiter=',', fmt='%1.8f')
    ...
    def simple_fourier(self):
        ...
        for j, rigid in enumerate(self.tracked_objects):
            ...
            fourier_mag = np.fft.fft(self.mag_tracker[:, j])
            fourier_mag = np.fft.fftshift(fourier_mag)
            freq = np.fft.fftfreq(self.time_axis.shape[-1], d=self.t)
            freq = np.fft.fftshift(freq)
            ax.plot(freq, np.absolute(fourier))
            ax.plot(freq, np.absolute(fourier_mag))
            fourier = np.abs(fourier)
            fourier_mag = np.abs(fourier_mag)
            ...
            fm = fourier_mag[int(0.5*self.extent/self.t):int((1/self.t)+(0.5*self.extent/self.t))]
            np.savetxt('3DOFFourier.csv', freq, delimiter=',', fmt='%1.8f')
            np.savetxt(f'Fourier{j}.csv', f, delimiter=',', fmt='%1.6f')
            np.savetxt(f'Fouriermag{j}.csv', fm, delimiter=',', fmt='%1.6f')
            ax.set_xlim((0, 0.5))
        frequency_plot.show()
...
Instance1 = Instance(TrialMesh, 0.001, 200)
Instance1.initialize_displacement([[0, 2, 0]], [[0, 0, 0.05]])
